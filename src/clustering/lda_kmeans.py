...
def assignment_file(filename, delim = '\t'):
    ...
    with open(filename) as source:
        ...
        for line in source:
            ...
            for i in xrange(2, len(entries)):
                if i % 2 == 0:
                    distribution[int(entries[i])] = float(entries[i+1])
            distribution = [distribution[i] for i in sorted(distribution.keys())]
            distributions.append(distribution)

    return cluster_ids, distributions

def train_infer_lda(train_data, test_data, K = 10, tmp_path = './tmp/'):

    n_iter = min(len(train_data), 1000)
    n_iter = max(800, n_iter)
    ...
    c_ids, distributions = assignment_file(train_topic, '\t')

    return c_ids, distributions
...
def main(argv):
    ...
    #sclusterID = train_infer_lda(trainX, testX, clusterN)
    _, distributions = train_infer_lda(trainX + testX, [], 10)

    #print distributions

    trainX, testX = distributions[:len(trainX)], distributions[len(trainX) : ]
